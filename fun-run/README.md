...
- Registers **multiple participants** in one submission (name, age, t-shirt size per runner, plus one contact per group).
...
- Appends one row per participant to the Google Sheet: `Bib #, First Name, Last Name, Age, T-Shirt Size, Contact Name, Contact Email, Contact Phone, Registered At`.
...
- **Shirt sizes**: `SHIRT_SIZES` in `Index.html`.
